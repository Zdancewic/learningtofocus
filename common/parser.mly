...
let m_r a b = merge_range (fst a) (fst b)

%}

%token <Range.t * string> IDENT
%token <Range.t * string> UIDENT
%token <Range.t * string> LITERAL
%token <Range.t * int32> INT

%token <Range.t> KW_INCLUDE
%token <Range.t> KW_FOF
%token <Range.t> KW_AXIOM 
%token <Range.t> KW_HYPOTHESIS
%token <Range.t> KW_DEFINITION
%token <Range.t> KW_ASSUMPTION
%token <Range.t> KW_LEMMA
%token <Range.t> KW_THEOREM
%token <Range.t> KW_CONJECTURE
%token <Range.t> KW_NEG_CONJECTURE
%token <Range.t> KW_PLAIN
%token <Range.t> KW_FI_DOMAIN
%token <Range.t> KW_FI_FUNCTORS
%token <Range.t> KW_FI_PREDICATES
%token <Range.t> KW_TYPE
%token <Range.t> KW_UNKNOWN

%token <Range.t> TOK_LPAREN
%token <Range.t> TOK_RPAREN 
%token <Range.t> TOK_COMMA 
%token <Range.t> TOK_QUESTION 
%token <Range.t> TOK_DOT 
%token <Range.t> TOK_COLON 
%token <Range.t> TOK_PERCENT 
%token <Range.t> TOK_LBRACKET 
%token <Range.t> TOK_RBRACKET 
%token <Range.t> TOK_BANG 
%token <Range.t> TOK_BANGEQ
%token <Range.t> TOK_AMPERSAND 
%token <Range.t> TOK_BAR 
%token <Range.t> TOK_TILDE 
%token <Range.t> TOK_LTEQ 
%token <Range.t> TOK_EQGT 
%token <Range.t> TOK_LTEQGT 
%token <Range.t> TOK_LTTILDEGT 
%token <Range.t> TOK_TILDEBAR 
%token <Range.t> TOK_TILDEAMPERSAND 
%token <Range.t> TOK_STAR 
%token <Range.t> TOK_PLUS 
%token <Range.t> TOK_TRUE
%token <Range.t> TOK_FALSE
