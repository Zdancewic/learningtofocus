...
module type S = sig
  ...
  val unif_empty : tm_unification
  val unif_combine : tm_unification -> tm_unification -> tm_unification option
...
(* Both have the same structure, making unification easier. *)
  ...
  let unif_empty : tm_unification = []
  let unif_single (t : Top.tag) (e : tm) : tm_unification =
    [(t, e)]
  let unif_combine (u1 : tm_unification) (u2 : tm_unification) : tm_unification option =
    List.fold_right
      (fun (tag, tm) ->
         function
         | None -> None
  ...
  let rec unify_terms (l1 : tm list) (l2 : tm list) : tm_unification option =
    List.fold_left2 (fun acc t1' t2' ->
        let t_unif_opt = (unify_term t1' t2') in
        begin match acc, t_unif_opt with
          | None, _ | _, None -> None
          | Some unif, Some t_unif -> unif_combine unif t_unif
        end)
      (Some unif_empty) l1 l2
  ...
  let unify_goal (g1:goal) (g2:goal) : prop_unification option =
    begin match g1, g2 with
      | Any, g | g, Any -> Some { any = Some g; terms = unif_empty }
  ...
  let rec apply_unif_tm (uni : tm_unification) (t : tm) : tm =
